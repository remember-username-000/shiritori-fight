...
function playerSubmit (wordUnit) {
    ...
    gameLog(`${playerObj.getName()} played ${wordUnit.word.kana[wordUnit.kanaIndex].text}, dealing ${10} damage to ${opponentObj.getName()}!`);
...
function computerSubmit () {
    if (findNextWord(wordsPlayed[wordsPlayed.length - 1]) == null) {
        gameLog('No more valid words available.');
        gameLog('Player wins!');
        endGame();
        return;
    } else {
        ...
        gameLog(`${opponentObj.getName()} played ${nextWord.word.kana[nextWord.kanaIndex].text}, dealing ${10} damage to ${playerObj.getName()}!`);
...
function startGame () {
    playerObj = new Player(100, 1, "Player");
